...
export default function Layout({
  ...
  const [searchword, { loading, data }] =
    useMutation<SearchResponse>("/api/search");
  ...
  return (
    <div>
      ...
      <div className="hidden md:flex h-[4rem] max-w-[75rem] mx-auto my-0 justify-between items-center py-3 px-4">
        ...
        <div>
          <form
            ...
            className="py-10 px-4 flex space-x-3"
          >
            <input
              {...register("search")}
      ...
      <div className="md:hidden bg-white h-12 w-full max-w-xl  justify-center text-lg px-10 font-medium fixed text-gray-800 border-b top-0  flex items-center z-10">
        {/* md:max-w-full */}
        {canGoBack ? (
      ...
      <div
        className={cls(
          "pt-12 md:max-w-5xl md:mx-auto md:my-0",
          hasTabBar ? "pb-24" : ""
        )}
      >
        {children}
      ...
      {hasTabBar ? (
        <nav className="md:hidden bg-white w-full  max-w-xl text-gray-700 border-t fixed bottom-0 px-10 pb-5 pt-3 flex justify-between text-xs">
          {/* md:max-w-full */}
          <Link href="/">
            <a
              className={cls(
